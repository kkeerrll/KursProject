...
class Abstract_action(ABC):
    ...
    def delete_file(self, vacancy: Vacancy):
        pass
...
class JSONSaver(Abstract_action):
    ...
    @staticmethod
    def delete_file():
        if os.path.exists(JSONSaver.file_name):
            os.remove(JSONSaver.file_name)


    @staticmethod
    def get_vacancies():
        with open(JSONSaver.file_name, 'a', encoding='utf-8') as file:
